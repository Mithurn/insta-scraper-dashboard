...
class ProxyInstagramScraper:
    def __init__(self):
        self.session = requests.Session()
        self.ua = UserAgent()
        self.rate_limit_delay = (2, 8)  # Random delay between 2-8 seconds
    ...
    def _try_instagram_api_with_proxy(self, username: str) -> Optional[Dict]:
        """Try Instagram API with proxy rotation"""
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if 'graphql' in data and 'user' in data['graphql']:
                        ...
                        if result and result.get('followers_count', 0) > 0:
                            logger.info(f"✅ Successfully scraped {username} via API with proxy")
                            return result
                ...
                logger.debug(f"API failed for {username}: {response.status_code}")
                
        except Exception as e:
            logger.debug(f"API error for {username}: {e}")
        return None

    def _try_instagram_web_with_proxy(self, username: str) -> Optional[Dict]:
        """Try web scraping with proxy"""
        try:
            ...
            if response.status_code == 200:
                ...
                for script in scripts:
                    try:
                        ...
                        if 'mainEntity' in data and 'additionalProperty' in data['mainEntity']:
                            ...
                            if result and result.get('followers_count', 0) > 0:
                                logger.info(f"✅ Successfully scraped {username} via web scraping with proxy")
                                return result
                    except:
                        continue
                ...
                if result and result.get('followers_count', 0) > 0:
                    logger.info(f"✅ Successfully scraped {username} via meta tags with proxy")
                    return result
    ...
    def scrape_profile(self, username: str) -> Optional[Dict]:
        """Main scraping method with proxy support and multiple fallbacks"""
        if not username:
            return None
        ...
        methods = [
            self._try_instagram_api_with_proxy,
            self._try_instagram_web_with_proxy
        ]
    ...
    def scrape_multiple_profiles(self, usernames: List[str]) -> Dict[str, Dict]:
        """Scrape multiple profiles with intelligent batching and proxy rotation"""
        results = {}
